...
COLUMN description FORMAT A70 WORD_WRAP
COLUMN usage FORMAT A98;
...
FROM (
  ...
  SELECT 'session.sql' AS name, 'Display session details ' AS description, '@session.sql' AS usage FROM dual UNION ALL
  SELECT 'session_count.sql' AS name, 'Display session details ' AS description, '@session_count.sql' AS usage FROM dual UNION ALL
  SELECT 'schema_obj_counts.sql' AS name, 'Display schema objects count' AS description, '@schema_obj_counts.sql <username> ' AS usage FROM dual UNION ALL
  SELECT 'cpu_info.sql ' AS name, 'Display Current CPU Usage' AS description, '@cpu_info.sql ' AS usage FROM dual UNION ALL
  SELECT 'dba_dir.sql' AS name, 'Display Database dictionarys ' AS description, '' AS usage FROM dual UNION ALL
  SELECT 'db.sql' AS name, 'Display Current database details ' AS description, '@db.sql' AS usage FROM dual UNION ALL
  SELECT 'datapump_job.sql' AS name, 'Display datapump Job  details ' AS description, '@datapump_job.sql' AS usage FROM dual UNION ALL
  SELECT 'db_size.sql' AS name, 'Dispaly Database Size ' AS description, '@db_size.sql' AS usage FROM dual UNION ALL
  SELECT 'dt.sql' AS name, 'Dispaly Current DATE and TIME of database ' AS description, '@dt' AS usage FROM dual UNION ALL
  SELECT 'fra.sql' AS name, 'Display fast recovery area space USAGE ' AS description, '@fra.sql' AS usage FROM dual UNION ALL
  SELECT 'fuzzy.sql' AS name, 'Show  fuzzy status on datafile ' AS description, '@fuzzy.sql' AS usage FROM dual UNION ALL
  SELECT 'gc_blocks_lost.sql' AS name, 'GC block lost details ' AS description, '@gc_blocks_lost.sql' AS usage FROM dual UNION ALL
  SELECT 'racdiag.sql' AS name, 'This we have to run at OS level as a oracle user ' AS description, '@racdiag.sql' AS usage FROM dual UNION ALL
  SELECT 'objs.sql' AS name, 'Display object details ' AS description, '@objs.sql <Object_name>' AS usage FROM dual UNION ALL
  SELECT 'tb_size.sql' AS name, 'calculate table size ' AS description, '@tb_size.sql <tb_name>' AS usage FROM dual UNION ALL
